...
class ToolHead:
    ...
    # Print time tracking
    def _flush_kinematics(self, lazy=False):
    ...
    def _process_moves(self, moves):
        ...
        while 1:
            self.print_time = min(self.print_time + batch_time, next_move_time)
            self._flush_kinematics(lazy=True)
            if self.print_time >= next_move_time:
                break
            if is_drip_mode:
    ...
    def dwell(self, delay):
        self.print_time = self.get_last_move_time() + max(0., delay)
        self._flush_kinematics(lazy=True)
        self._check_stall()
    def motor_off(self):
